...
| `active_subgraph_limit` / `relation_breadth_limit` | 4 / `None` | `cascade.py:CascadeConfig`, `select_active_constraint_subgraph` | eligible-node selection | maximum same-hop active nodes | NAMED_DEFAULT / NAMED_CONFIG | EFFECTIVE when configured | BASIC_DERIVED / relation breadth | finite active view; does not change hop, ranking, weights, or M_B relations |
| `relevance_floor` | 0.05 | `cascade.py:select_active_constraint_subgraph` | seed-node selection | minimum query relevance | NAMED_DEFAULT | EFFECTIVE | ADVANCED_TUNABLE / relation selection | candidate admission floor; separate from Basic breadth cap |
| `relation_hop` | 1 | `cascade.py` | explicit relation propagation | traversal depth | STRUCTURAL_FIXED | CONSUMED | CURRENT_BOUNDARY_GUARD / advanced relation depth | current one-hop boundary, not a Basic breadth setting |
...
| `knowledge_update_threshold` | `None` | `runtime.py:EnterpriseRuntime` | pre-admission durability score gate | Basic destruction-test admission boundary | NAMED_DEFAULT | EFFECTIVE when explicit | BASIC_DERIVED / knowledge update caution | does not bypass mandatory durability, shadow, Authority, or irreversible safety; success is not Truth |
| `success_reinforcement_gain` | `None` | `runtime.py:EnterpriseRuntime` → `mb_graph.py:MBNode` | successful existing-node feedback | bounded support increase | NAMED_DEFAULT | EFFECTIVE when explicit | ADVANCED / success reinforcement | does not alter success classification, confidence, Candidate admission, or safety gates; event provenance retained |
