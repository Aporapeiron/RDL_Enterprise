...
class TestManualSedimentationBenchmark(unittest.TestCase):
    def test_same_manual_and_case_set_are_used_without_label_leakage(self):
        ...
        self.assertTrue(all("trace_case_ids" in row for row in results))
        self.assertEqual(results[0]["relation_consumed_count"], 0)
        self.assertEqual([row["relation_consumed_count"] for row in results], [0, 0, 0])
        self.assertEqual([row["relation_effect_count"] for row in results], [0, 0, 0])
        self.assertTrue(all("NOT_EVALUATED" in row["relation_effect_status"] for row in results))
        self.assertTrue(all(row["relation_effect_count"] == 0 for row in results))
